...
export const saveUserProfileToFirestore = async (
  ...
  try {
    ...
    await setDoc(
      settingsRef,
      { ...profile, userId, updatedAt: new Date().toISOString() },
      { merge: true },
    );
  } catch (err) {
...
// Diet Profiles & System Prompts: users/{userId}/diet_profiles/{profileId}
export const subscribeToDietProfiles = (
  userId: string,
  callback: (profiles: UserProfile[]) => void,
) => {
  verifyAuthUser(userId);
  const path = `users/${userId}/diet_profiles`;
  const profilesRef = collection(db, "users", userId, "diet_profiles");
  return onSnapshot(
    profilesRef,
    (snapshot) => {
      const profiles: UserProfile[] = [];
      snapshot.forEach((docSnap) => {
        profiles.push({ ...(docSnap.data() as UserProfile), id: docSnap.id });
      });
      callback(profiles);
    },
    (err) => {
...
export const saveDietProfileToFirestore = async (
  userId: string,
  profile: UserProfile,
) => {
  verifyAuthUser(userId);
  ...
  const path = `users/${auth.currentUser.uid}/diet_profiles/${profile.id}`;
  try {
    ...
    console.log(
      "Firestore write succeeded for user profile:",
      auth.currentUser.uid,
    );
  } catch (error) {
...
export const deleteDietProfileFromFirestore = async (
  userId: string,
  profileId: string,
) => {
  verifyAuthUser(userId);
  if (!auth.currentUser) {
    console.error("Firestore delete failed: No authenticated user");
    throw new Error("User is not authenticated.");
  }
  const path = `users/${userId}/diet_profiles/${profileId}`;
  try {
    const profileRef = doc(db, "users", userId, "diet_profiles", profileId);
    await deleteDoc(profileRef);
    console.log(
      "Firestore delete succeeded for user profile:",
      auth.currentUser.uid,
    );
  } catch (error) {
...
export const initializeDefaultDietProfiles = async (
  userId: string,
  defaultProfiles: UserProfile[],
) => {
  verifyAuthUser(userId);
  for (const prof of defaultProfiles) {
    await saveDietProfileToFirestore(userId, prof);
  }
};
...
export const saveMealPlanEntryToFirestore = async (
  userId: string,
  entry: MealPlanEntry,
) => {
  verifyAuthUser(userId);
...
export const deleteMealPlanEntryFromFirestore = async (
  userId: string,
  entryId: string,
) => {
  verifyAuthUser(userId);
...
export const savePantryItemToFirestore = async (
  userId: string,
  item: PantryItem,
) => {
  verifyAuthUser(userId);
...
export const deletePantryItemFromFirestore = async (
  userId: string,
  itemId: string,
) => {
  verifyAuthUser(userId);
...
export const saveAdHocOverrideToFirestore = async (
  userId: string,
...
export const getAdHocOverrideFromFirestore = async (
  userId: string,
): Promise<AdHocOverride | null> => {
  verifyAuthUser(userId);
  const path = `users/${userId}/profile/adhoc`;
  try {
    const overrideRef = doc(db, "users", userId, "profile", "adhoc");
